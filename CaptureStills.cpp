...
int main(int argc, char* argv[])
{
     ...
     *
     *
An application performing a standard streaming capture operation should perform the
following steps:
...
IDeckLinkInput::DoesSupportVideoMode to check if the combination of the video
...
*/
    ...
    //result is an HRESULT, GetDeckLinkIterator is an HRESULT that takes as an argument an IDeckLinkIterator and is S_OK if Decklink cards are detected)

    if (result != S_OK)
		goto bail;

    //bail ends everything, releases selectedDeckLinkInput and deckLinkIterator
...
*/
    ...
    //strcmp = string compare(first string, second string) 0= they are the same, <0 = 	the first character that does not match has a lower value in ptr1 than in ptr2, >0 = the first character that does not match has a greater value in ptr1 than in ptr2
